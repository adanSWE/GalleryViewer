...
public class ImageRecord {
	...
	public void setImageDate(LocalDate dateParameter) {
		if (dateParameter == null) {
			...
			// Splitting the days,months, and year to check the length of each string
			String[] individualDateElements = formattedDate.split("-"); // making an array of individual date elements
																		// (day, month, year)

			// Creating another String variable to hold the formatted Date (DD-MM-YYYY)
			String currentDate = "";

			for (int i = 0; i < individualDateElements.length; i++) { // Going through each element in the individual
																		// date elements
				// Checking for the length of the DAYS string, if its two, that means its double
				// digits, if its one, that means its a single digit, therefore i can add a 0,
				// to exactly match the sample data in the question.
				if ((i == 0) && (individualDateElements[0].length() == 1)) {
