...
public class ImageManager {
	...
	public ImageManager() {
		...
		initialize(); // Initializing all the pre-existing records in the arrayList each time the program is ran.
	}

	// Object behaviour methods:
	
	/**
	 * This method stores all the data that pre-exists in the arraylist
	 */
	...
	public void addImage(ImageRecord image) {
		...
		ImageAlbum imgAlbum = new ImageAlbum(arrayList.toArray(new ImageRecord[0])); // The new record is made into an array and then the record is give to the newly instantiated ImageAlbum object
	}
	...
	public ImageAlbum searchGenre(ImageType type) {
		ArrayList<ImageRecord> foundRecords = new ArrayList<ImageRecord>(); // Initializing an arrayList "foundRecords", to store all the found records that have ImageType of type, "type"
		for(int i =0; i < arrayList.size(); i++) { // Using a for loop to go through the ImageRecord ArrayList "arrayList" that holds all Image Records.
			ImageRecord record = arrayList.get(i); // Assigning each record to a variable called "record" one by one.
